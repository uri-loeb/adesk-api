...
serializer = URLSafeSerializer(SIGNING_SECRET, salt="adesk")

# ====== APP ======
app = Flask(__name__)
# CORS is not strictly required when using Netlify proxy, but keep it restricted for safety
...
def make_svg_data_url(width: int, height: int, text: str) -> str:
    svg = f"""<svg xmlns='http://www.w3.org/2000/svg' width='{width}' height='{height}'>
      ...
      <text x='50%' y='50%' dominant-baseline='middle' text-anchor='middle'
            font-family='Inter, Arial, sans-serif' font-size='{max(24, min(width, height)//16)}'
            fill='#0f172a'>{text}</text>
    </svg>"""
    return "data:image/svg+xml;base64," + base64.b64encode(svg.encode()).decode()

def make_copy_50_words(product: str, variant: int) -> str:
    base = (f"{product} – fast, simple, and designed for real results. "
            f"Get three crisp ad variations, a bold headline, and clean visuals tailored to your chosen size. "
            f"No logos or trademarks, just brand-agnostic impact. "
            f"Variation {variant} focuses on clarity, value, and a strong call to action that converts.")
    words = base.split()
    if len(words) < 50:
...
def payment_success():
    ...
    sess = stripe.checkout.Session.retrieve(
        session_id, expand=["customer_details", "payment_intent"]
    )
    ...
    # UPDATED: redirect only with ?token=...  (no &backend=... needed anymore)
...
def api_generate():
    ...
    for i in range(1, 4):
        ...
        copy50 = make_copy_50_words(product, i)
...
# (Optional) webhook (left unchanged)
@app.post("/webhook")
def webhook():
    ...
    try:
        event = stripe.Webhook.construct_event(request.data, sig, wh_secret)
    except Exception:
        return "invalid", 400
